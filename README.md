...
The project i,plements a calendar system that returns the dates while adjusting to:
...
2. _Structure_
   The project is divided into different parts based on if its domain (core models), policy (validation and rules) or service (logic and calculations). This desicion was made to ensure it follows a clean architecture (even if its a small project).
...
4. _Handling fractions of a workday_
   Decimal values are handled seperatly from whole values. I.e. 2.5 is calculated as 2 whole workdays and 0.5 workday.
...
The solution includes tests written based on the examples provided as well as some misc tests written during the development phase. This case was solved with a testdriven approach.
